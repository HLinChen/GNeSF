...
from tools.semantic_mapper import PALETTE_21
...
class TSDFVolume:
    ...
    def __init__(self, vol_bnds, voxel_size, use_gpu=True, margin=5, enable_sem=True):
        """Constructor.
        ...
        self._sem_vol_cpu = -np.ones(self._vol_dim).astype(np.float32) # if enable_sem else None
        ...
        if self.gpu_mode:
            ...
            # if enable_sem:
            self._sem_vol_gpu = cuda.mem_alloc(self._sem_vol_cpu.nbytes)
            self.cuda.memcpy_htod(self._sem_vol_gpu, self._sem_vol_cpu)
            # else:
            #     self._sem_vol_gpu = None
        ...
        __global__ void integrate(float * tsdf_vol,
          ...
          // Intergrate sem
          // float old_sem = sem_vol[voxel_idx];
          float new_sem = sem_im[pixel_y*im_w+pixel_x];
          sem_vol[voxel_idx] = new_sem;
          return;
    ...
    def integrate(self, color_im, depth_im, cam_intr, cam_pose, obs_weight=1., sem_im=None):
        ...
        if sem_im is not None:
            # Fold RGB color image into a single channel image
            sem_im = sem_im.astype(np.float32)
            sem_im = sem_im.reshape(-1).astype(np.float32)
        else:
            sem_im = np.array(0)
        ...
        if self.gpu_mode:  # GPU mode: integrate voxel volume (calls CUDA kernel)
            ...
            if sem_im is not None:
                ...
                self._sem_vol_cpu[valid_vox_x, valid_vox_y, valid_vox_z] = new_sem # newest label wins
    ...
    def get_volume(self):
        if self.gpu_mode:
            ...
            self.cuda.memcpy_dtoh(self._sem_vol_cpu, self._sem_vol_gpu)
        
        if self.enable_sem:
            return self._tsdf_vol_cpu, self._color_vol_cpu, self._weight_vol_cpu, self._sem_vol_cpu
        else:
    ...
    def get_mesh(self):
        ...
        """
        if self.enable_sem:
            tsdf_vol, color_vol, weight_vol, sem_vol = self.get_volume()
        else:
            tsdf_vol, color_vol, weight_vol = self.get_volume()
        ...
        if self.enable_sem:
            sem_vals = sem_vol[verts_ind[:, 0], verts_ind[:, 1], verts_ind[:, 2]]
            # sems = np.floor(np.asarray([sem_vals])).T
            sems = sem_vals.astype(np.int8)
            return verts, faces, norms, colors, sems
        else:
...
def meshwrite(filename, verts, faces, norms, colors, sems=None):
    ...
    for i in range(verts.shape[0]):
        if sems is None:
            ...
            color = PALETTE_21[sems[i]]
            
            # print(sems[i])
            ply_file.write("%f %f %f %f %f %f %d %d %d %d\n" % (
                verts[i, 0], verts[i, 1], verts[i, 2],
                norms[i, 0], norms[i, 1], norms[i, 2],
