...
class ToTensor(object):
    ...
    def __call__(self, data):
        ...
        if 'semantics' in data.keys():
            data['semantics'] = torch.tensor(np.stack(data['semantics']), dtype=torch.long)
        return data
...
class IntrinsicsPoseToProjection(object):
    ...
    def __call__(self, data):
        ...
        for intrinsics, extrinsics in zip(data['intrinsics'], data['extrinsics']):
            view_proj_matrics = []
            for i in range(3): # multi-scale
                # from (camera to world) to (world to camera)
                proj_mat = torch.inverse(extrinsics.data.cpu()) # world to camera
...
def pad_scannet(img, intrinsics):
    ...
    w, h = img.size
    if w == 1296 and h == 968:
        img = ImageOps.expand(img, border=(0, 2), fill=0)
        intrinsics[1, 2] += 2
    return img, intrinsics


def pad_scannet_semantic(sem, fill=0):
    """ Scannet images are 1296x968 but 1296x972 is 4x3
    so we pad vertically 4 pixels to make it 4x3
    """
    ...
    w, h = sem.shape
    if w == 1296 and h == 968:
        sem = cv2.copyMakeBorder(src=sem, top=2, bottom=2, left=0, right=0, borderType=cv2.BORDER_CONSTANT, value=fill)
    return sem
    ...
    def __call__(self, data):
        for i, im in enumerate(data['imgs']):
            ...
            if 'semantics' in data:
                semantics = pad_scannet_semantic(data['semantics'][i], fill=self.ignore_label)
                semantics = cv2.resize(semantics, self.size, interpolation=cv2.INTER_NEAREST)
                data['semantics'][i] = semantics
...
class RandomTransformSpace(object):
    ...
    def __init__(self, voxel_dim, voxel_size, random_rotation=True, random_translation=True,
        ...
        self.origin = origin # [0, 0, 0]
        self.voxel_size = voxel_size # 0.04
        self.random_rotation = random_rotation # True
        self.random_translation = random_translation # True
        self.max_depth = max_depth # 3.0
        self.padding_start = torch.Tensor([paddingXY, paddingXY, paddingZ]) # [0.1000, 0.1000, 0.0250]
        # no need to pad above (bias towards floor in volume)
        self.padding_end = torch.Tensor([paddingXY, paddingXY, 0]) # [0.1000, 0.1000, 0.0]

        # each epoch has the same transformation
        self.random_r = torch.rand(max_epoch) # 50
    ...
    def __call__(self, data):
        ...
        if (not self.random_rotation) and (not self.random_translation):
            ...
            # construct rotaion matrix about z axis; why random?
            ...
            # randomly sample a crop; why random?
        ...
        data = self.transform(data, T.inverse(), old_origin=origin) # local
    ...
    def transform(self, data, transform=None, old_origin=None,
        ...
        # ----------computing visual frustum hull------------ local
        bnds = torch.zeros((3, 2)) # minimal, maximal; local boundary
        bnds[:, 0] = np.inf
        bnds[:, 1] = -np.inf
        ...
        for i in range(data['imgs'].shape[0]):
            ...
            view_frust_pts = get_view_frustum(self.max_depth, size, cam_intr, cam_pose) # [3, 5]
        ...
        if 'tsdf_list_full' in data.keys():
            ...
            for l, tsdf_s in enumerate(data['tsdf_list_full']): # multi-scale
                ...
                coords_world_s = coords.view(3, x, y, z)[:, ::2 ** l, ::2 ** l, ::2 ** l] / 2 ** l # multi-scale sample
                ...
                coords_world_s = coords_world_s[[2, 1, 0]].T.view([1] + dim_s + [3]) # bs, x, y, z, 3
...
def get_view_frustum(max_depth, size, cam_intr, cam_pose):
    ...
    im_h = int(im_h)
    im_w = int(im_w)
    view_frust_pts = torch.stack([ # (3, 5) 5 points: origin + corners of image
