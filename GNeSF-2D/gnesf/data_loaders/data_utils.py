...
def random_flip(rgb, camera, src_rgbs, src_cameras, sem=None, depth=None, src_sems=None):
    h, w = rgb.shape[:2]
    ...
    # out = [rgb_out, camera, src_rgbs, src_cameras]
    if sem is not None:
        sem_out = np.flip(sem, axis=1).copy()
        out['sem'] = sem_out
        # out.append(sem_out)
    if depth is not None:
        depth_out = np.flip(depth, axis=1).copy()
        out['depth'] = depth_out
        # out.append(depth_out)
    if src_sems is not None:
        src_sems = np.flip(src_sems, axis=-1).copy()
        out['src_sems'] = src_sems
    return out
