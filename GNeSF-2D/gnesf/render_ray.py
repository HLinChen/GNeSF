...
def raw2outputs(raw, z_vals, mask, white_bkgd=False, enable_rgb=False, enable_semantic=False):
    '''
    ...
    '''
    sigma = raw['sigma']
    if enable_rgb: rgb = raw['rgb']
    if enable_semantic: sem = raw['sem']
    ...
    if enable_semantic:
        sem_map = torch.sum(weights.unsqueeze(2) * sem, dim=1)  # [N_rays, 21]

    if white_bkgd:
        if enable_rgb:
            rgb_map = rgb_map + (1. - torch.sum(weights, dim=-1, keepdim=True))
        if enable_semantic:
            sem_map = sem_map - (1. - torch.sum(weights, dim=-1, keepdim=True))
    ...
    if enable_rgb: ret['rgb'] = rgb_map
    if enable_semantic: ret['sem'] = sem_map
...
def render_rays(ray_batch,
    ...
    out = projector.compute(pts, ray_batch['camera'],
                            ray_batch['src_rgbs'],
                            ray_batch['src_cameras'],
                            featmaps=featmaps[0],
                            sem_pred2d=sem_pred2d)  # [N_rays, N_samples, N_views, x]
    rgb_feat_sampled, ray_diff, mask = out[:3]
    if sem_pred2d is not None: sem_sampled = out[-1]
    else: sem_sampled = None
    pixel_mask = mask[..., 0].sum(dim=2) > 1   # [N_rays, N_samples], should at least have 2 observations

    raw_coarse = model.net_coarse(rgb_feat_sampled, ray_diff, mask, sem_sampled)   # [N_rays, N_samples, 4]
    ...
    if N_importance > 0:
        ...
        raw_fine = model.net_fine(rgb_feat_sampled, ray_diff, mask, sem_sampled)
