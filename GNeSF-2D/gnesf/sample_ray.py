...
class RaySamplerSingleImage(object):
    def __init__(self, data, device, resize_factor=1, render_stride=1):
        ...
        self.sem = data['sem'] if 'sem' in data.keys() else None
        self.depth = data['depth'] if 'depth' in data.keys() else None
        self.src_sems = data['src_sems'] if 'src_sems' in data.keys() else None
        ...
        if resize_factor != 1:
            ...
            if self.sem is not None:
                self.sem = F.interpolate(self.sem.unsqueeze(1), scale_factor=resize_factor, mode='nearest').squeeze(1)
            if self.depth is not None:
                self.depth = F.interpolate(self.depth.unsqueeze(1), scale_factor=resize_factor, mode='nearest').squeeze(1)
        ...
        if self.sem is not None:
            self.sem = self.sem.reshape(-1)
        if self.depth is not None:
            self.depth = self.depth.reshape(-1)
    ...
    def get_all(self):
        ...
        if self.sem is not None: ret['sem'] = self.sem.cuda()
        if self.depth is not None: ret['depth'] = self.depth.cuda()
        if self.src_sems is not None: ret['src_sems'] = self.src_sems.cuda()
        return ret
    ...
    def random_sample(self, N_rand, sample_mode, center_ratio=0.8):
        ...
        if self.sem is not None: ret['sem'] = sem.cuda()
        if self.depth is not None: ret['depth'] = depth.cuda()
        if self.src_sems is not None: ret['src_sems'] = self.src_sems.cuda()
        return ret
