...
class Projector():
    ...
    def compute(self,  xyz, query_camera, train_imgs, train_cameras, featmaps, sem_pred2d=None):
        '''
        ...
        :param sem_pred2d: [n_views, 21, h, w]
        ...
        if sem_pred2d is not None:
            sem_sampled = F.grid_sample(sem_pred2d, normalized_pixel_locations, align_corners=True)
            sem_sampled = sem_sampled.permute(2, 3, 0, 1)  # [n_rays, n_samples, n_views, 21]
        ...
        out = [rgb_feat_sampled, ray_diff, mask]
        if sem_pred2d is not None: out.append(sem_sampled)
        return out
