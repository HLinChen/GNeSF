...
class IBRNet(nn.Module):
    def __init__(self, args, in_feat_ch=32, n_samples=64, **kwargs):
        ...
        if args.enable_rgb or args.enable_semantic:
    ...
    def forward(self, rgb_feat, ray_diff, mask, sem_pred2d=None):
        '''
        ...
        out = {'sigma': sigma_out.squeeze(-1)}
        
        
        if self.args.enable_rgb or self.args.enable_semantic:
            x = torch.cat([x, vis, ray_diff], dim=-1)
            x = self.weight_fc(x)
            x = x.masked_fill(mask == 0, -1e9)
            blending_weights_valid = F.softmax(x, dim=2)  # sem blending
        
        # sem computation x
        if self.args.enable_semantic:
            sem_out = torch.sum(sem_pred2d * blending_weights_valid, dim=2)

            out['sem'] = sem_out

        if self.args.enable_rgb:
            # rgb computation
