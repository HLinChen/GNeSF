...
#                'mirror', 'floor', 'clothes', 'ceiling', 'books', 'refrigerator',
#                'tv', 'paper', 'towel', 'shower curtain', 'box', 'white board',
#                'person', 'night stand', 'toilet', 'sink', 'lamp', 'bathtub',
#                'bag', 'other struct', 'otherfurn', 'other prop') # 
...
def _get_scene(dir='./data', mode='val'):
    file_path = '{}/scannetv2_{}.txt'.format(dir, mode.replace('train_eval', 'train'))
    # file_path = 'data/debug.txt'
    # file_path = 'data/scannetv2_train_split.txt' # s-ray !!!
    # file_path = 'data/scannetv2_{}_split.txt'.format(mode.replace('train_eval', 'train')) # s-ray !!!
    # file_path = 'data/scannetv2_{}_split3.txt'.format(mode.replace('train_eval', 'train')) # s-ray !!!
...
def load_scannet_semantic_json(data_root, mode='train'):
    ...
    # if 'train' not in mode: scenes: scenes[:10] # s-ray !!!
    # scenes = scenes[:60] if 'train' in mode else scenes[:10] # s-ray !!!
    # scenes = scenes[:100] if 'train' in mode else scenes[:10]
    # scenes = scenes[:200] if 'train' in mode else scenes[:10]
    # scenes = scenes[:500] if 'train' in mode else scenes[:10]
    # scenes = scenes[:800] if 'train' in mode else scenes[:10]
    # scenes = scenes[:1000] if 'train' in mode else scenes[:10]
    
    logger.info('len of scenes: {}'.format(len(scenes)))
    
    # with open('/data/hlchen/code/NeuralRecon/data/scannet_{}.json'.format(mode)) as f:
    # with open('data/scannet_{}.json'.format(mode), 'r') as f:
    with open('data/scannet_val_split.json', 'r') as f: # s-ray !!!
...
_RAW_SCANNET_SPLITS_STEP = {
    ...
    # "scannet_test": ("val", "scans_test", "color", "label-filt"),
}
...
_RAW_SCANNET_SPLITS = {
    ...
    "scannet_sem_seg_train_eval": ("train_eval"),
    # "scannet_test": ("val", "scans_test", "color", "label-filt"),
}
